...
def content_rewrite(mbus, new_value):
    ...
    if (command == 1) and (length == 6):  # reading
        ...
        if CoilVal == '\x00\x00':
            slave_state[CoilAddr] = 0
        else:
            slave_state[CoilAddr] = 1
...
def content_mask(mbus):
    ...
    if (command == 1) and (length >= 4):  # reading
        ...
        for x in range(-1, -1 - len(trans_dict[TransID]), -1):
            ...
            if x == -1:
                if num == 0:
                    BinaryStr = BinaryStr[:x] + '0'
                elif num == 1:
                    BinaryStr = BinaryStr[:x] + '1'
            else:
                if num == 0:
                    BinaryStr = BinaryStr[:x] + '0' + BinaryStr[x+1:]
                elif num == 1:
                    BinaryStr = BinaryStr[:x] + '1' + BinaryStr[x+1:]
                # else: do nothing
        ...
        if CoilAddr not in slave_state:
            return mbus
        if slave_state[CoilAddr] == 0:
            mbus = mbus[:10] + '\x00\x00'
        else:
            mbus = mbus[:10] + '\xFF\x00'
